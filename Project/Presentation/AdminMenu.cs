public class AdminMenu : EmployeeMenu
{
    ...
    private static void RemoveEmployeeAccount()
    {
        ...
        if (accounts.Count() == 0)
        {
            OptionsMenu.Logo("delete accounts");
            Console.WriteLine("No deletable Accounts were found");
        ...
        else
        {
            ...
            {
                if (account.Type == AccountModel.AccountType.EMPLOYEE || account.Type == AccountModel.AccountType.ADMIN || account.Type == AccountModel.AccountType.CUSTOMER)
                {
            ...
            int option = OptionsMenu.DisplaySystem(employeeList, "delete accounts", "Choose what account you want to remove.");
